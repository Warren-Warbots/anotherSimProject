...
public class RobotManager extends SubsystemBase {
  ...
  public Command setModeCommand(RobotState state, boolean isAuto) {
    // in auto, we want the snap command to end as soon as we are in a non snap
    // state
    // in teleop, we want it to stick around (aka dont run disable snap) so that the
    // snap doesnt cancel itself until the driver says otherwise
